...
void ConnectionToWindowServer::paint(i32 window_id, Gfx::IntSize window_size, i32 scale_factor, Vector<Gfx::IntRect> const& rects)
{
    if (auto* window = Window::from_window_id(window_id))
        Core::EventLoop::current().post_event(*window, make<MultiPaintEvent>(rects, window_size, scale_factor));
}

void ConnectionToWindowServer::window_target_buffer_change(i32 window_id, Gfx::IntSize new_size, i32 scale_factor)
{
    if (auto* window = Window::from_window_id(window_id)) {
