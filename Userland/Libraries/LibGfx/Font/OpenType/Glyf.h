...
public:
    ...
    public:
        ...
        RefPtr<Gfx::Bitmap> rasterize(i16 font_ascender, i16 font_descender, float x_scale, float y_scale, Gfx::GlyphSubpixelOffset subpixel_offset, i32 scale, GlyphCb glyph_callback) const
        {
            switch (m_type) {
            case Type::Simple:
                return rasterize_simple(font_ascender, font_descender, x_scale, y_scale, subpixel_offset, scale);
            case Type::Composite:
                return rasterize_composite(font_ascender, font_descender, x_scale, y_scale, subpixel_offset, scale, glyph_callback);
            }
            VERIFY_NOT_REACHED();
        }
    ...
    private:
        ...
        RefPtr<Gfx::Bitmap> rasterize_simple(i16 ascender, i16 descender, float x_scale, float y_scale, Gfx::GlyphSubpixelOffset, i32 scale) const;
        ...
        template<typename GlyphCb>
        RefPtr<Gfx::Bitmap> rasterize_composite(i16 font_ascender, i16 font_descender, float x_scale, float y_scale, Gfx::GlyphSubpixelOffset subpixel_offset, i32 scale, GlyphCb glyph_callback) const
        {
            u32 width = (u32)(ceilf((m_xmax - m_xmin) * x_scale)) + 1;
            u32 height = (u32)(ceilf((font_ascender - font_descender) * y_scale)) + 1;
            Gfx::PathRasterizer rasterizer(Gfx::IntSize(width, height), scale);
